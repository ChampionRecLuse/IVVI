...
class env(py_environment.PyEnvironment):
    ...
    def __init__(self, env_name, rand_seed, misc_info):
        ...
        self.action_space = box.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32)
        self.observation_space = box.Box(low=-np.inf, high=np.inf, shape=(1,), dtype=np.float32)
        self.transition = np.array([[ 0.33067476, -0.14593205, -0.18204439 , 0.14348094,  0.17652493, -0.00286107, -0.17045097]])
        # Initialize the start observation
        self._old_ob = np.array([0])
    ...
    def _step(self, action):
        ...
        if self._done:
            self._reset()
        feature = np.hstack(([1], self._old_ob, action, self._old_ob ** 2, action ** 2, self._old_ob ** 3, action ** 3))
        # get the observation
        ob = np.dot(self.transition, feature) + np.random.normal()
    ...
    def reward(self, data_dict):
        ...
        return -0.05 * (data_dict['start_state'][0] ** 2)
