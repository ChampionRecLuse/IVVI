...
from brac import agent
from brac import networks
...
class Agent(agent.Agent):
  ...
  def __init__(
      self,
      env='Instrumental_Variable_Value_Iteration',
  ...
  def _build_test_policies(self):
    ...
    # policy = policies.DecoupleDeterministicSoftPolicy2(
    #     a_embed=self._agent_module.p_embed,
    #     a_network=self._agent_module.p_net)
  ...
  def _build_online_policy(self):
    ...
    # return policies.DecoupleRandomSoftPolicy2(
    #     a_embed=self._agent_module.p_embed,
    #     a_network=self._agent_module.p_net,
    #     )
  ...
  def restore(self, ckpt_name):
    ...
    self._checkpointer['q_embed'].restore(ckpt_name)
    self._checkpointer['p_embed'].restore(ckpt_name)
    self._checkpointer['g_net'].restore(ckpt_name)
    self._checkpointer['f_net'].restore(ckpt_name)
