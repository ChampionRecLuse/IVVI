...
def train_eval_ebm(
  ...
  test_env_name = env_name + '_test'
  tf_env_test = train_eval_utils.env_factory(test_env_name)
  ...
  collector = train_eval_utils.DataCollector(tf_env, explore_policy, train_data, env_name)
  ...
  collector = train_eval_utils.DataCollector(
      tf_env, agent.online_policy, train_data, env_name)
  interval = 1
  for _train_step in range(total_train_steps):
    ...
    if step % eval_freq == 0 or step == total_train_steps:
      ...
      eval_result, eval_infos = train_eval_utils.eval_policies(
          tf_env_test, agent.test_policies, n_eval_episodes, step)
