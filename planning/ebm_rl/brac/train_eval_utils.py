...
def eval_policy_episodes(env, policy, n_episodes, step):
...
def eval_policies(env, policies, n_episodes, step):
  results_episode_return = []
  infos = collections.OrderedDict()
  for name, policy in policies.items():
    mean, _ = eval_policy_episodes(env, policy, n_episodes, step)
...
class DataCollector(object):
  ...
  def __init__(self, tf_env, policy, data, env_name, eps_portion=0.):
    self._tf_env = tf_env
    self._policy = policy
    self._data = data
  ...
  def collect_transition(self):
    ...
    # if self.env_name == 'recommender' or 'recommenderbaseline' or 'recommenderopt' or 'recommender_test' or 'recommenderbaseline_test' or 'recommenderopt_test':
    #     self._saved_action = tf.cast(self._saved_action, dtype=tf.float32)
    ...
    # if self.env_name == 'recommender' or 'recommenderbaseline':
    #     action = tf.cast(action, dtype=tf.int32)
    #     if action.dtype != 'int32':
    #         print('!')
    #         print(action.dtype)


    # Important! Use action to get the next time step (observation) We may need to change this!
    next_time_step = self._tf_env.step(action)
    next_action = self._policy(next_time_step.observation)[0]
    # if self.env_name == 'recommender' or 'recommenderbaseline' or 'recommenderopt' or 'recommender_test' or 'recommenderbaseline_test' or 'recommenderopt_test':
    #     if next_action.dtype != 'int32':
    #         # print(next_action)
    #         next_action = tf.cast(next_action > 0.5, dtype=tf.int32)
    #         # next_action = tf.cast(next_action, dtype=tf.int32)
    #         # print(next_action)
    #         # print('--------')
