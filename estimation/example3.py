...
# Use analytical form of K and use 400 data to estimate matrix A,B,C. Use gradient descent method to compute W.
...
def compute_W_sad2(phi, psi, eta_phi, eta_psi, x, a, z, xx, max_iter, W_0, K_0, size):
    ...
    for i in range(int(max_iter/size)):
        ...
        for t in range(size):
            # use one sample to estimate matrix A and C does not work
            # Phi = np.matrix(phi(x[:, i * size + t], a[:, i * size + t]))
            # Psi = np.matrix(psi(x[:, i * size + t], z[:, i * size + t]))
            # A = np.dot(Psi, np.transpose(Phi))
            # C = np.dot(xx[:, i * size + t], np.transpose(Psi))
            Gradient_front = np.dot(np.dot(np.transpose(A),np.linalg.inv(B)), A)
            Gradient_back = np.dot(C, np.dot(np.linalg.inv(B), A))
            Gradient = np.dot(W_now, Gradient_front) - Gradient_back
            W_next = W_now - eta_phi[i * size + t] * Gradient
            W_now = W_next.copy()
